...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Check if user has scrolled to bottom area (last 30% of page)
      const scrollTop = window.scrollY;
      const docHeight = document.documentElement.scrollHeight - window.innerHeight;
      const scrollPercent = docHeight > 0 ? (scrollTop / docHeight) * 100 : 0;

      // Show button when scrolled past 70% of page
  ...
  return (
    <a
      ...
      className={`fixed right-6 bottom-6 z-50 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full p-3 shadow-lg flex items-center justify-center transition-all duration-300 ${
